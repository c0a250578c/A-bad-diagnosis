...
const EVALUATION_MESSAGES = {
  ...
  GOOD: { threshold: 60, message: '全体的に良好です。一部改善の余地あり 👍' },
  NEEDS_IMPROVEMENT: { threshold: 40, message: '改善ポイントが見られます 📋' },
  POOR: { threshold: 0, message: '生活習慣の見直しをお勧めします 💪' }
} as const;
...
export function getEvaluationMessage(score: number): string {
  ...
  if (score >= EVALUATION_MESSAGES.NEEDS_IMPROVEMENT.threshold) {
    return EVALUATION_MESSAGES.NEEDS_IMPROVEMENT.message;
  }
  return EVALUATION_MESSAGES.POOR.message;
}
