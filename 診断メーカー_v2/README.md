...
> **6つの生活習慣カテゴリ**（睡眠・学習・スマホ・計画性・運動/健康・人間関係）を診断し、Gemini AIによるパーソナライズされた個別アドバイスを提供するReactアプリケーション
...
| 開始画面 | 診断中 | 結果表示 |
|:---:|:---:|:---:|
| 診断の概要と履歴表示 | 24問のインタラクティブ診断 | AI個別分析 + レーダーチャート |

---

## ✨ 主な機能

- **6カテゴリ×24問の総合診断** - 学生生活の全側面を詳細にカバー
- **🤖 AI個別アドバイザー** - Gemini AIが診断結果を読み解き、個別の改善策を提案
- **インタラクティブな可視化** - Rechartsによるレーダーチャート表示
- **診断履歴管理** - LocalStorageでの永続化
- **アニメーションUX** - CSSアニメーションによるスムーズな遷移

---

## 🔑 環境変数（APIキー）の設定

AIアドバイス機能を有効にするには、`.env` ファイルに以下の設定が必要です：
